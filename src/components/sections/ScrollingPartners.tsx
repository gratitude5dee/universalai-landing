      ...
      <div className="relative">
        ...
        '--duration': '20s'
          ...
          <div className="flex shrink-0 animate-marquee">
            ...
            {[...Array(3)].map((_, setIndex) => <div key={`set-${setIndex}`} className="flex shrink-0">
                {logos.map(logo => <div key={`${setIndex}-${logo.id}`} className="mx-12 flex items-center justify-center w-32 h-16" style={{
              transform: `scale(${logo.scale || 1})`
            }}>
                    <img src={logo.src} alt={logo.name} className="max-h-10 w-auto object-contain grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-300" />
                  </div>)}
              </div>)}
          </div>
        </div>
